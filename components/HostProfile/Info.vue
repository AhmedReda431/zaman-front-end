...
const { updateUser, loading, error, errors: serverErrors } = useUpdateUser();
...
const formSchema = ref({
    fields: [
        ...
        },
        {
            ...
            name: 'company',
            as: 'input',
            placeholder: t('companyOrActivity'),
        },
        {
            label: t('occupation'),
            name: 'occupation',
            as: 'input',
            placeholder: t('occupations'),
        },
        {
            label: t('bio'),
            name: 'bio',
...
<template>
    <div class="update-container">
        ...
        <DynamicForm :server-error="serverErrors" :formValues="user" class="w-4/6 pt-6" :schema="formSchema" @submit="handleUpdate">
            <template #buttuns>
                <!--Submit button-->
                <div class="pt-6 text-center">
                    <button v-if="!loading"
                        class="mb-3 inline-block bg-zaman hover:bg-zaman-800 w-full rounded-md px-6 py-3 text-xs font-medium uppercase leading-normal text-white transition duration-150 ease-in-out focus:outline-none focus:ring-0"
