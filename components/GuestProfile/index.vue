...
import ProfileReservations from './Reservations.vue'
...
<template>
    <section class="mx-auto grid md:grid-cols-8 max-w-7xl p-6 gap-10 lg:px-8">
        <!-- start mobile menu -->
        <ClientOnly>
            <Menu as="div" class="relative ml-3 md:hidden block  ">
                ...
                <transition enter-active-class="transition ease-out duration-100"
                    ...
                    <MenuItems
                        ...
                        <MenuItem>
                        <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                            :class="{ 'text-zaman bg-gray-100': toRaw(currentComponent) === ProfileInfo }"
                            @click="switchComponent(ProfileInfo)">
                        ...
                        <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                            ...
                            @click="switchComponent(ProfileReservations)">
                        ...
                        <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                            ...
                            @click="switchComponent(ProfileFavs)">
                        ...
                        <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                            ...
                            @click="switchComponent(ProfileNotifications)">
                        ...
                        <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                            ...
                            @click="switchComponent(ProfileSettings)">
        ...
        <div class="md:block hidden py-6 bg-white shadow-md rounded-lg col-span-2">
            <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                :class="{ 'text-zaman bg-gray-100': toRaw(currentComponent) === ProfileInfo }"
                @click="switchComponent(ProfileInfo)">
                <Icon size="24" :name="`material-symbols:person-2-${toRaw(currentComponent) === ProfileInfo ? '' : 'outline-'}rounded`" />
                {{ t('profile.personalInfo') }}
            </button>
            <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                :class="{ 'text-zaman bg-gray-100': toRaw(currentComponent) === ProfileReservations }"
            ...
            <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                :class="{ 'text-zaman bg-gray-100': toRaw(currentComponent) === ProfileFavs }"
                @click="switchComponent(ProfileFavs)">
                <Icon size="24" :name="`material-symbols:favorite${toRaw(currentComponent) === ProfileFavs ? '' : '-outline'}`" />
                {{ t('profile.favorites') }}
            </button>
            <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                :class="{ 'text-zaman bg-gray-100': toRaw(currentComponent) === ProfileNotifications }"
                @click="switchComponent(ProfileNotifications)">
                <Icon size="24" :name="`material-symbols:notifications${toRaw(currentComponent) === ProfileNotifications ? '' : '-outline'}`" />
                {{ t('profile.notifications') }}
            </button>
            <button class="block hover:bg-gray-100 w-full rtl:text-right px-4 py-3 rounded-sm" style="min-width: max-content"
                :class="{ 'text-zaman bg-gray-100': toRaw(currentComponent) === ProfileSettings }"
                @click="switchComponent(ProfileSettings)">
                <Icon size="24" :name="`material-symbols:settings${toRaw(currentComponent) === ProfileSettings ? '' : '-outline'}`" />
                {{ t('profile.accountSettings') }}
            </button>
        </div>
        <div class="col-span-6">
            <transition name="fade" mode="out-in">
                <component class="flex flex-col items-center md:block bg-white shadow-md h-full rounded-lg p-6" :is="currentComponent" key="currentComponent">
                </component>
            </transition>
        </div>
    </section>
