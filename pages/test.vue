  ...
  <div class=" md:col-span-2">
    ...
    <div class="relative overflow-hidden rounded-lg mb-2">
      <div class="absolute top-0 right-0 p-2 flex space-x-2 rtl:space-x-reverse">
        <button v-for="tab in tabs" :key="tab" class="bg-white bg-opacity-80 text-gray-700 px-3 py-1 rounded-md text-sm hover:bg-opacity-100 transition">
          {{ tab }}
        </button>
      </div>
    ...
    <div class="flex space-x-2 rtl:space-x-reverse overflow-x-auto pb-2">
      <div v-for="(image, index) in images" :key="index" 
        ...
        <div v-if="index === 0" class="absolute top-0 left-0 bg-black bg-opacity-50 text-white px-1 rounded-tl text-xs">
          +{{ images.length }}
        </div>
      </div>
    </div>
...
<div class="flex justify-around space-x-4 rtl:space-x-reverse p-4 border-t border-b border-gray-200">
  ...
  <button v-for="(icon, index) in contactIcons" :key="index" class="text-gray-600 hover:text-gray-800 flex items-center">
